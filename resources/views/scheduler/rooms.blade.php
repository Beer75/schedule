...
@section('load_function')
    onLoad="start('rooms','{{ route('schedule.data')}}')"
@endsection
...
@section('content')
    <div class="schedule">
        <div class="s_row">
            <div class="sc_title">Кабинет</div>
            @php
        ...
        @foreach ($rooms as $room)
        <div class="s_row">
            <div class="sr_title">{{$room->number}}</div>
            @foreach ($periods as $period)
                <div class="sc_lesson" data-period="{{$period->id}}" data-room="{{$room->id}}"></div>
            @endforeach
        </div>
        @endforeach
